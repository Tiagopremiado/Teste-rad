...
interface PlanManagementProps {
  users: User[];
  grantPremium: (uid: string, durationInDays: number | 'lifetime') => void;
  revokePremium: (uid: string) => void;
}

const getPremiumStatus = (user: User) => {
    if (user.is_lifetime) return { text: 'Vitalício', isPremium: true, expiryDate: null };
    if (user.premium_expiry) {
        const expiry = new Date(user.premium_expiry).getTime();
        if (Date.now() < expiry) {
            return { text: `Expira em ${new Date(expiry).toLocaleDateString('pt-BR')}`, isPremium: true, expiryDate: new Date(expiry) };
        }
    }
    return { text: 'Grátis', isPremium: false, expiryDate: null };
};

const PlanManagement: React.FC<PlanManagementProps> = ({ users, grantPremium, revokePremium }) => {
    ...
    const filteredUsers = useMemo(() => {
        ...
        ).slice(0, 5); // Limit results to avoid a huge list
    }, [users, searchTerm]);
    ...
    const handleAction = (id: string, action: 'grant' | 'revoke', duration?: number | 'lifetime') => {
        const user = users.find(u => u.id === id);
        if (!user) return;

        const actionText = action === 'revoke' ? 'revogar o acesso Premium de' : `conceder acesso ${duration === 'lifetime' ? 'Vitalício' : `${duration} dias`} para`;
        if (window.confirm(`Tem certeza que deseja ${actionText} ${user.display_name}?`)) {
            if (action === 'grant' && duration !== undefined) {
                grantPremium(id, duration);
            ...
            // Force re-render of selected user info by re-selecting them from the main users list
            setSelectedUser(prev => users.find(u => u.id === prev?.id) || null);
        }
    };
    ...
    const premiumStatus = selectedUser ? getPremiumStatus(selectedUser) : null;

    return (
        <div className="p-6 space-y-6">
            <h2 className="text-2xl font-bold text-white">Gerenciamento de Planos Premium</h2>
            ...
            <div className="relative max-w-md">
                <input
                    ...
                    className="w-full bg-gray-700 text-white placeholder-gray-400 p-3 rounded-lg border border-gray-600 focus:ring-2 focus:ring-amber--- START OF FILE hooks/useSocialData.ts ---
